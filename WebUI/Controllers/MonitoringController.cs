    ...
    public class MonitoringController : Controller
    {
        ...
        public JsonResult GetHourData(int groupID, bool flat = true)
        {
            ...
            if (flat)
            {
                ...
                {
                    var values = repository.GetTagValue(tag.TagName, DateTime.Today, DateTime.Today.AddDays(1)).ToList();
            ...
            {
                var normtag = group.Tags.Where(t => t.TagName.Trim().EndsWith(NORM)).FirstOrDefault();
                var facttag = group.Tags.Where(t => t.TagName.Trim().EndsWith(FACT)).FirstOrDefault();
        ...
        {
            var group = repository.GetGroups().Where(g => g.GroupID == groupID).First();
            var taglimit = group.Tags.FirstOrDefault(t => t.TagName.Trim().EndsWith(LIMIT));
            var tagfact = group.Tags.FirstOrDefault(t => t.TagName.Trim().EndsWith(FACT));
        ...
        public ActionResult Note(int groupID = 0, DateTime? date = null)
        {
            ...
            var normtag = search.Group.Tags.Where(t => t.TagName.Trim().EndsWith(NORM)).FirstOrDefault();
            var facttag = search.Group.Tags.Where(t => t.TagName.Trim().EndsWith(FACT)).FirstOrDefault();

            if (normtag == null)
            {
                var errorinfo = new HandleErrorInfo(new Exception("Отсутствует тэг по нормативу"), "Monitoring", "Note" );
                return View("Error", errorinfo);
            }
            if (facttag == null)
            {
                var errorinfo = new HandleErrorInfo(new Exception("Отсутствует тэг по факту"), "Monitoring", "Note");
                return View("Error", errorinfo);
            }
